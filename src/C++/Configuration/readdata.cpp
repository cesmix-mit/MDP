...
int implReadConfigStruct(configstruct &config, string filein, Int mpiprocs, Int mpirank, Int backend)
{
    ...
    readarray(in, &config.ws, config.nsize[13]);   
    readarray(in, &config.stress, config.nsize[14]);   
    config.pbc = readiarrayfromdouble(in, config.nsize[15]);
    config.move = readiarrayfromdouble(in, config.nsize[16]);
    config.group = readiarrayfromdouble(in, config.nsize[17]);    
...
void implSetDomainStruct(domainstruct &dom, commonstruct &common, regionstruct &reg, latticestruct &lat)
{
    ...
    if (common.readlattice==0) {
        dom.triclinic = common.reg.triclinic;
        for (int i = 0; i< 3; i++) {
            dom.boxhi[i] = common.reg.boxhi[i]; 
            dom.boxlo[i] = common.reg.boxlo[i]; 
            dom.boxtilt[i] = common.reg.boxtilt[i]; 
            dom.boxlo_lamda[i] = 0.0;
            dom.boxhi_lamda[i] = 1.0;                    
        }
        ...
        dstype epsilon[3]; 
        for (int i=0; i<3; i++) epsilon[i] = 1e-6*(dom.boxhi[i]-dom.boxlo[i]);
        ...
        dom.triclinic = common.reg.triclinic;
        dom.boxtilt[0] = common.lat.spacing[0]*common.reg.boxtilt[0];                     
        dom.boxtilt[1] = common.lat.spacing[0]*common.reg.boxtilt[1];                     
        dom.boxtilt[2] = common.lat.spacing[1]*common.reg.boxtilt[2];                                     
        for (int i = 0; i< 3; i++) {
            dom.boxhi[i] = common.lat.spacing[i]*common.reg.boxhi[i]; 
            dom.boxlo[i] = common.lat.spacing[i]*common.reg.boxlo[i];        
            dom.boxlo_lamda[i] = 0.0;
            dom.boxhi_lamda[i] = 1.0;                    
        }                          
        ...
        cpuBoundingSubbox(dom.bsublo, dom.bsubhi, dom.sublo, dom.subhi,   
                dom.sublo_lamda, dom.subhi_lamda, dom.boxlo, dom.h, dom.triclinic);                                        
        cpuLatticeBoundingBox(common.lat.sublo, common.lat.subhi, dom.bsublo, dom.bsubhi, 
...
void implSetDomainStruct(domainstruct &dom, commonstruct &common, configstruct &config, int ci)
{
    int dim = common.dim;
    ...
    dom.boxlo[1] = 0.0;
    dom.boxlo[2] = 0.0;    
    dom.boxhi[0] = config.a[dim*ci];
    dom.boxhi[1] = config.b[dim*ci+1];
    dom.boxhi[2] = config.c[dim*ci+2];        
    dom.boxtilt[0] = config.b[dim*ci];
    dom.boxtilt[1] = config.c[dim*ci];
    dom.boxtilt[2] = config.c[dim*ci+1];    
    
    if ((fabs(dom.boxtilt[0]) > 1e-10) || (fabs(dom.boxtilt[1]) > 1e-10) || (fabs(dom.boxtilt[2]) > 1e-10))
        dom.triclinic = 1;
    else
        dom.triclinic = 0;
...
void implSetConfigStruct(configstruct &config, appstruct &app, commonstruct &common)
{
    int dim = common.dim;
    int mpiRank = common.mpiRank;
    int backend = 1;    
    int nlocal = config.natoms[0];    
    ...
    if (app.nsize[59]>0) {  // setatomtypefraction      
    ...
    if (app.nsize[58]>0) { // createvelocity
        int *ilist;
        dstype *amass;                
        TemplateMalloc(&ilist, nlocal, backend);
...
void implSetConfigStruct(configstruct &config, domainstruct &dom, appstruct &app, commonstruct &common, regionstruct &reg, latticestruct &lat)
{
    ...
    int nlocal = cpuAtomAdd(config.x, config.t, y, dom.h_inv, dom.boxlo, dom.sublo_lamda,
        dom.subhi_lamda, ilist, common.dim, 0, common.lat.natom);
...
{        
    ...
    if (common.readdomain==0) {
        if (common.readconfig==0) 
            ...
            implSetDomainStruct(common.dom, common, config, 0);    
    }    
    if (common.readconfig==0) {
        implSetConfigStruct(config, common.dom, app, common, common.reg, common.lat);            
    }
    else
        implSetConfigStruct(config, app, common);            
    ...
    if (mpirank==0)
        printf("Done with ReadInputFiles...\n");   
