...
void cpuEmbeddingEnergy(dstype *f, dstype *df, dstype *ei, int inum) 
{
    
}
void cpuComputeEamEnergyForce(dstype *ei, dstype *eij, dstype *fij, dstype *xij, dstype *qi, dstype *qj, int *ti, int *tj, int *ai, int *aj, 
        dstype *pairparam, int *pairnum, int *pairnumsum, int dim, int ncq, int nparam, int inum, int ijnum, int potnum)
{
    dstype *f = &eij[0]; 
    dstype *df = &eij[inum];     
    
    cpuComputePairEnergyForce(eij, fij, xij, qi, qj, ti, tj, ai, aj, pairparam, dim, ncq, nparam, ijnum, potnum); 
    cpuElectronDensity(ei, eij, pairnum, pairnumsum, inum); 
    cpuEmbeddingEnergy(f, df, ei, inum); 
...
void cpuComputeTripletEnergyForce(dstype *eij, dstype *fij, dstype *fik, dstype *xij, dstype *xik, dstype *qi, dstype *qj, dstype *qk, 
     int *ti, int *tj, int *tk, int *ai, int *aj, int *ak, dstype *tripletparam, int dim, int ncq, int nparam, int ijknum, int potnum)
{
}
...
{        
    for (Int b=0; b<common.nba; b++) {
        ...
        cpuComputePairForce(fij, xij, qi, qj, ti, tj, ai, aj, param, dim, ncq, nparam, ntuples, potnum);
    ...
    for (Int b=0; b<common.nba; b++) {
        ...
        if (dim==2) {
            if (decomp==0)
                ...
                Int *p0 = &tmp.intmem[1+3*na+3*ntuples]; // ntuples       
                Int *p1 = &tmp.intmem[1+3*na+4*ntuples]; // ntuples       
                Int *p2 = &tmp.intmem[1+3*na+5*ntuples]; // ntuples       
                Int *p3 = &tmp.intmem[1+3*na+6*ntuples]; // ntuples       
                Int naj = cpuUniqueSort(jlist, bnumsum, index, p0, aj, p1, p2, p3, ntuples);
...
void implEamForce(dstype *f, neighborstruct &nb, commonstruct &common, appstruct &app, tempstruct &tmp, 
        dstype* x, dstype *q, dstype* param, dstype rcutsq, Int *atomtype, Int nparam, Int typei, Int typej, Int decomp, Int potnum)
{        
    for (Int b=0; b<common.nba; b++) {
        Int e1 = common.ablks[b];
        ...
        dstype *ei = &tmp.tmpmem[ntuples*(2*dim+2*ncq+1)]; // ntuples
        cpuComputeEamEnergyForce(ei, eij, fij, xij, qi, qj, ti, tj, ai, aj, param, pairnum, pairnumsum, 
                dim, ncq, nparam, na, ntuples, potnum);
...
void implTersoffForce(dstype *f, neighborstruct &nb, commonstruct &common, appstruct &app, tempstruct &tmp, 
...
{        
    for (Int b=0; b<common.nba; b++) {
        ...
        cpuEmbeddingEnergy(c3ij, d3ij, h3ij, npairs); 
...
{        
    for (Int b=0; b<common.nba; b++) {
        ...
        Int *temp = &tmp.intmem[1 + 4*na + na*jnum]; //  (ilist, tripletnum, tripletnumsum, pairnum, pairlist, tmp)            
