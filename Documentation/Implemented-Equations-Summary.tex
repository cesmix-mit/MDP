...
In order to reduce the computations needed to calculate the derivative of the bispectrum basis functions, Eq. \ref{eq:bs}, neighbor information is considered. Thus, the bispectrum basis functions that should be implemented are:

\begin{equation}
    d_{tkk'l{l_1}{l_2}}(\bm r^{N_j}, j, i) = \sum_{s \in \Omega'_{jit}} b_{skk'l{l_1}{l_2}}(\bm r^{N_j}, j, i)
\end{equation}

where $\Omega'''_{jt}$ is replaced with  $\Omega'_{jit}$.
