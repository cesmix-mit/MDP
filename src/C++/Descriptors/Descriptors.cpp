...
{        
    ...
    for (Int b=0; b<common.nba; b++) {
        ...
        dstype *xij = &tmp.tmpmem[ntuples*(10+dim)]; // ntuples*dim
        dstype *qi;
        dstype *qj;
        cpuNeighPairs(xij, qi, qj, x, q, ai, aj, ti, tj, pairnum, pairlist, pairnumsum, ilist, nb.alist, 
                atomtype, na, jnum, ncq, dim);       
                        
//         dstype *sr = &tmp.tmpmem[ntuples*(dim)];   // ntuples
//         dstype *si = &tmp.tmpmem[ntuples*(dim+1)]; // ntuples
//         dstype *srx = &tmp.tmpmem[ntuples*(dim+2)]; // ntuples
//         dstype *sry = &tmp.tmpmem[ntuples*(dim+3)]; // ntuples
//         dstype *srz = &tmp.tmpmem[ntuples*(dim+4)]; // ntuples
//         dstype *six = &tmp.tmpmem[ntuples*(dim+5)]; // ntuples
//         dstype *siy = &tmp.tmpmem[ntuples*(dim+6)]; // ntuples
//         dstype *siz = &tmp.tmpmem[ntuples*(dim+7)]; // ntuples
        
        dstype *sr = &tmp.tmpmem[ntuples*(8)];   // ntuples
        dstype *si = &tmp.tmpmem[ntuples*(9)]; // ntuples
        dstype *srx = &tmp.tmpmem[ntuples*(2)]; // ntuples
        dstype *sry = &tmp.tmpmem[ntuples*(3)]; // ntuples
        dstype *srz = &tmp.tmpmem[ntuples*(4)]; // ntuples
        dstype *six = &tmp.tmpmem[ntuples*(5)]; // ntuples
        dstype *siy = &tmp.tmpmem[ntuples*(6)]; // ntuples
        dstype *siz = &tmp.tmpmem[ntuples*(7)]; // ntuples                
        ...
        dstype *cr = &tmp.tmpmem[0]; // ntuples
        dstype *ci = &tmp.tmpmem[ntuples]; // ntuples        
        dstype *ei = &tmp.tmpmem[ntuples*8]; // na * Nbf
        ...
        // e = e + ei^T*onevec        
        dstype *onevec =  &tmp.tmpmem[ntuples*8 + na*Nbf];   // na
        PGEMTV(common.cublasHandle, na, Nbf, &one, ei, na, onevec, inc1, &one, e, inc1, common.backend);        
        
        dstype *fij = &tmp.tmpmem[ntuples*8]; // dim * ntuples * Nbf
    ...
    if (natomtypes==1) {
        ...
        if (common.chemtype == 0) {
            for (int i = 0; i < natomtypes; i++)
                SphericalHarmonicBesselDescriptors(&e[i*Nbf], &f[i*inum*Nbf], nb, common, app, tmp, sh, 
                        x, q, param, &app.rcutsqml[0], nb.atomtype, nparam, i+1, 0, common.decomposition);                                    
        }
        else {
            for (int i = 0; i < natomtypes; i++)
                for (int j = 0; j < natomtypes; j++)
                    SphericalHarmonicBesselDescriptors(&e[(j+i*natomtypes)*Nbf], &f[(j+i*natomtypes)*inum*Nbf], nb, common, app, tmp, sh, x, q, 
...
{        
    ...
    for (Int b=0; b<common.nba; b++) {
        ...
        dstype *cr = &tmp.tmpmem[0]; // ntuples
        dstype *ci = &tmp.tmpmem[ntuples]; // ntuples       
        dstype *ei = &tmp.tmpmem[2*ntuples]; // na 
        dstype *di = &tmp.tmpmem[ntuples*(dim+8)]; // na * Nbf
        ...
        dstype *dd = &tmp.tmpmem[ntuples*(dim+8)]; // dim * ntuples * Nbf
        ...
        PGEMNV(common.cublasHandle, dim*ntuples, Nbf, &one, dd, dim*ntuples, coeff, inc1, &one, fij, inc1, common.backend);    
