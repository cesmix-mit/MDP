...
if gen==0
    ...
    tmp = strgpu;    
    tmp = replace(tmp, "(T *u, T *xij, T *xik," => "Gradient(T *u, T *du, T *u_xij, T *u_xik, T *xij, T *xik,");            
    tmp = replace(tmp, "(double *," => "Gradient(double *, double *, double *, double *,");            
    tmp = replace(tmp, "(float *," => "Gradient(float *, float *, float *, float *,");            
    strgpu = strgpu * "\n" * tmp;        
else
    stropu = "template <typename T> void " * opufile;
    ...
    st2 = replace(st2, "(T *u, T *xij," => "Gradient(T *u, T *du, T *u_xij, T *u_xik, T *xij,");                    
