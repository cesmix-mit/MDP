...
//         printArray2D(x, common.dim, common.inum, common.backend);
    }
    else {
        for (int i = 0; i < common.npot2b; i++)
            if (common.atom2b[2*i] == common.atom2b[1+2*i]) {
...
{    
    common.bondtype = 0; // non-bonded interaction    
    for (int i = 0; i < common.npot1a; i++)
        cpuSingleEnergyForce(&e[i*inum], &f[i*dim*inum], nb, common, app, tmp, x, q, param, nb.atomtype, nparam, 0, common.pot1a[i]);            
...
{    
    common.bondtype = 1; // non-bonded interaction    
    for (int i = 0; i < common.npot1b; i++)
        cpuSingleEnergyForce(&e[i*inum], &f[i*dim*inum], nb, common, app, tmp, x, q, param, nb.atomtype, nparam, common.atom1b[i], common.pot1b[i]);                
}
...
{    
    ...
    if (common.neighpair == 0) {
        for (int i = 0; i < common.npot2a; i++)
            cpuFullNeighPairEnergyForceVirial(&e[i*inum], &f[i*dim*inum], &v[i*6*inum], nb, common, app, tmp, x, q, param, &app.rcutsq2a[i], 
                nb.atomtype, nparam, 0, 0, common.decomposition, common.pot2a[i]);                    
    }
    else {
        for (int i = 0; i < common.npot2a; i++)
            cpuHalfNeighPairEnergyForceVirial(&e[i*inum], &f[i*dim*inum], &v[i*6*inum], nb, common, app, tmp, x, q, param, &app.rcutsq2a[i], 
                nb.atomtype, nparam, 0, 0, common.decomposition, common.pot2a[i]);                    
    }        
}
...
{    
    ...
    if (common.neighpair == 0) {
        for (int i = 0; i < common.npot2b; i++) {
            if (common.atom2b[2*i] == common.atom2b[1+2*i]) {
                cpuFullNeighPairEnergyForceVirial(&e[i*inum], &f[i*dim*inum], &v[i*6*inum], nb, common, app, tmp, x, q, param, &app.rcutsq2b[i], 
                    nb.atomtype, nparam, common.atom2b[2*i], common.atom2b[1+2*i], common.decomposition, common.pot2b[i]);            
            }
            else {
                cpuFullNeighPairEnergyForceVirial(&e[i*inum], &f[i*dim*inum], &v[i*6*inum], nb, common, app, tmp, x, q, param, &app.rcutsq2b[i], 
        ...
        for (int i = 0; i < common.npot2b; i++)
            if (common.atom2b[2*i] == common.atom2b[1+2*i]) {
                cpuHalfNeighPairEnergyForceVirial(&e[i*inum], &f[i*dim*inum], &v[i*6*inum], nb, common, app, tmp, x, q, param, &app.rcutsq2b[i], 
                    nb.atomtype, nparam, common.atom2b[2*i], common.atom2b[1+2*i], common.decomposition, common.pot2b[i]);                    
            }
            else {
                cpuHalfNeighPairEnergyForceVirial(&e[i*inum], &f[i*dim*inum], &v[i*6*inum], nb, common, app, tmp, x, q, param, &app.rcutsq2b[i], 
...
{    
    ...
    int k = 0;
    if (common.npot1a > 0)
        cpuNonbondedSingleEnergyForce(&ei[m], &f[n], nb, common, app, tmp, x, q, &param[nparam[0]], dim, inum, nparam[1]-nparam[0]);              

    m += common.npot1a*inum;
    n += common.npot1a*dim*inum;
    if (common.npot1b > 0)
        cpuBondedSingleEnergyForce(&ei[m], &f[n], nb, common, app, tmp, x, q, &param[nparam[1]], dim, inum, nparam[2]-nparam[1]);              

    m += common.npot1b*inum;
    n += common.npot1b*dim*inum;    
    if (common.npot2a > 0)
        cpuNonbondedPairEnergyForceVirial(&ei[m], &f[n], &v[k], nb, common, app, tmp, x, q, &param[nparam[2]], dim, inum, nparam[3]-nparam[2]);              

    m += common.npot2a*inum;
    n += common.npot2a*dim*inum;
    k += common.npot2a*6*inum;
    if (common.npot2b > 0)
        cpuBondedPairEnergyForceVirial(&ei[m], &f[n], &v[k], nb, common, app, tmp, x, q, &param[nparam[3]], dim, inum, nparam[4]-nparam[3]);              
