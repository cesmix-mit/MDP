function [config,app] = readconfig(app, mode)
...
app.ncx = ncx; % number of compoments of x
...
n2 = 6;
m  = (1+dim*dim)*nconfigs;
n1 = n2+1;
n2 = n2+m;    
tm = reshape(tmp(n1:n2),[nconfigs (1+dim*dim)]); 
...
% n1 = n2+1;
% n2 = n2+m;
% config.natom = reshape(tmp(n1:n2),[m 1]); % number of atoms per each configuration  
% config.natomall = sum(config.natom);% number of atoms for all configurations
% 
