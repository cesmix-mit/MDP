...
function gentripletgrad(foldername,filename, u, xij, xik, qi, qj, qk, ti, tj, tk, ai, aj, ak, mu, eta, kappa, gen, ifile)
...
if gen==0
    ...
    stropu = stropu * tmp * "{\n";
    stropu = stropu * "}\n";
    stropu = replace(stropu, "(T *u, T *xij," => "(T *u, T *u_xij, T *u_xik, T *xij,");    

    tmp = "template void " * opufile;
    tmp = tmp * sp1;
    tmp = tmp * "template void " * opufile;
    tmp = tmp * sp2;
    tmp = replace(tmp, "(double *," => "(double *, double *, double *,");            
    tmp = replace(tmp, "(float *," => "(float *, float *, float *,");                
    ...
    strgpu = strgpu * "\t}\n" * "}\n\n";
    strgpu = replace(strgpu, "(T *u, T *xij" => "(T *u, T *u_xij, T *u_xik, T *xij");
    ...
    tmp = tmp * "}\n";
    tmp = replace(tmp, "(T *u" => "(T *u, T *u_xij, T *u_xik");    
    tmp = replace(tmp, "(u," => "(u, u_xij, u_xik,");  
    strgpu = strgpu * tmp;
