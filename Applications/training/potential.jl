...
function Paira(xij, qi, qj, ti, tj, mu, eta, kappa)
    ...
    tap0 = 1;  tap4 = -35/(Rcut^4);
    tap5 = 84/(Rcut^5); tap6 = -70/(Rcut^6);
    tap7 = 20/(Rcut^7);
...
function Tripletb(xij, xik, qi, qj, qk, ti, tj, tk, mu, eta, kappa)
    ...
    u = mu[1]*(costhe - mu[2])^2 + mu[3]*(mu[4] - rjk)^2;   
...
function Tripletc(xij, xik, qi, qj, qk, ti, tj, tk, rho, mu, eta, kappa)    
    ...
    m = mu[8];
    n = mu[9];        
    fA = -mu[1]*exp(-mu[2]*rij);
    fB = mu[4]*(1 + mu[5]^2/mu[6]^2 - mu[5]^2/(mu[6]^2 + (costhe-mu[7])^2));
    fC = (1 + mu[10]^n*rho[1]^n)^(-1.0/(2*n));
    u = [fA; fB*exp(mu[3]^m*(rij - rik)^m); fC]; 
