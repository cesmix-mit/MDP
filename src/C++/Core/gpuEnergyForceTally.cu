...
#ifndef __GPUENERGYFORCETALLY
#define __GPUENERGYFORCETALLY
...
template <typename T> __global__ void gpuKernelHalfForceDecomposition(T *f, T *fij, int *ai, int *aj, int dim, int ijnum)
{    
    int ii = threadIdx.x + blockIdx.x * blockDim.x;     
    while (ii < ijnum)  {  // for each atom pair ij in the simulation box     
        ...
        int l = dim*ii;
        for (int d = 0; d<dim; d++) {
            atomicAdd(&f[d+i], -fij[d+l]);
            atomicAdd(&f[d+j],  fij[d+l]);
        }
        ii += blockDim.x * gridDim.x;   
    }
}
...
{                        
    ...
    gpuKernelHalfForceDecomposition<<<gridDim, blockDim>>>(f, fij, ai, aj, dim, ijnum);
}
template void gpuHalfForceDecomposition(double*, double*, int*, int*, int, int);
template void gpuHalfForceDecomposition(float*, float*, int*, int*, int, int);

template <typename T> __global__ void gpuKernelIAtomDecomposition(T *f, T *fij, int *ilist, int *anumsum, int dim, int inum)
{    
    int ii = threadIdx.x + blockIdx.x * blockDim.x;     
    while (ii < inum)  {  // for each atom i in the simulation box     
...
{                        
    ...
    gpuKernelIAtomDecomposition<<<gridDim, blockDim>>>(f, fij, ilist, anumsum, dim, inum);
}
template void gpuIAtomDecomposition(double*, double*, int*, int*, int, int);
template void gpuIAtomDecomposition(float*, float*, int*, int*, int, int);


template <typename T> __global__ void gpuKernelJAtomDecomposition(T *f, T *fij, 
        int *jlist, int *bnumsum, int *index, int dim, int jnum)
{        
    int ii = threadIdx.x + blockIdx.x * blockDim.x;     
    while (ii < jnum)  {  // for each atom j in the simulation box     
...
template <typename T> void gpuJAtomDecomposition(T *f, T *fij, 
        int *jlist, int *bnumsum, int *index, int dim, int jnum)
{                        
    int blockDim = 256;
    int gridDim = (jnum + blockDim - 1) / blockDim;
    ...
    gpuKernelJAtomDecomposition<<<gridDim, blockDim>>>(f, fij, jlist, bnumsum, index, dim, jnum);
}
template void gpuJAtomDecomposition(double*, double*, int*, int*, int*, int, int);
template void gpuJAtomDecomposition(float*, float*, int*, int*, int*, int, int);
