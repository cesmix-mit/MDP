...
function genbo2grad(filename, u, xij, qi, qj, ti, tj, ai, aj, rho, mu, eta, kappa, natomtype)
...
for i = 1:m
    ...
    if ui(end) ~= 0
        potnum = potnum+1;        
        [stropu1, strcpu1, strgpu1] = genpairgrad(filename+num2str(potnum), ui(1:end-1), xij, qi, qj, ti, tj, ai, aj, mu, eta, kappa, gen, ifile);
        stropu1 = modifystr(stropu1, natomtype);
        strcpu1 = modifystr(strcpu1, natomtype);
        strgpu1 = modifystr(strgpu1, natomtype);
        strgpu1 = strrep(strgpu1, "\t\ti += blockDim.x * gridDim.x;\n\t\t}", "\t\t}\n\t\ti += blockDim.x * gridDim.x;");          
...
newstr = oldstr;
newstr = strrep(newstr, "u_xij[", "\tu_xij[");
for i = 1:natomtype
