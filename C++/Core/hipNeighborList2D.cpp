...
#ifndef __HIPNEIGHBORLIST2D
#define __HIPNEIGHBORLIST2D

#include "hipcub/device/device_radix_sort.hpp"

template <typename T>
__global__ void hipKernelGhostAtoms2D(int *glistnum, int *inside, T *x, T *pimages, T *wc, 
...
template <typename T>
__global__ void hipKernelAtomList2D(int *alist, int *inside, int *glistnumsum,
...
template <typename T> void hipAtomList2D(int *alist,  int *inside, int *glistnumsum, int *glistnum, 
...
{                        
    ...
    hipLaunchKernelGGL(hipKernelGhostAtoms2D, gridDim, blockDim, 0, 0, glistnum, inside, x, pimages, wc, s2rmap, inum, pnum, dim);
    
    // a list contains the starting position of the ghost atom of every atom i
    hipCumsum(glistnumsum, glistnum, d_sums, d_incr, inum+1); 

    hipLaunchKernelGGL(hipKernelAtomList2D, gridDim, blockDim, 0, 0, alist, inside, glistnumsum, x, pimages, inum, pnum, dim);
}
template void hipAtomList2D(int*, int*, int*, int*, int*, int*, double*, double*, double*, double*, int, int, int);
template void hipAtomList2D(int*, int*, int*, int*, int*, int*, float*, float*, float*, float*, int, int, int);

template <typename T>
__global__ void hipKernelCellList2D(int *clist, T *x, T *eta1, T *eta2, T *eta3, T *s2rmap, 
...
template <typename T> void hipCellList2D(int *clist, T *xi, T *eta1, T *eta2, T *eta3, T *s2rmap, 
        int *nc, int inum, int natom, int dim)
{                        
    int blockDim = 256;
    ...
    hipLaunchKernelGGL(hipKernelCellList2D, gridDim, blockDim, 0, 0, clist, xi, eta1, eta2, eta3, s2rmap, 
        nc, inum, natom, dim);
}
template void hipCellList2D(int*, double*, double*, double*, double*, double*, int*, int, int, int);
template void hipCellList2D(int*, float*, float*, float*, float*, float*, int*, int, int, int);

__global__ void hipKernelCellCounts(int *cellcounts, int* cell, int natom, int ncell)
{
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    while (i < natom) {              
...
void hipCellCounts(int *cellcounts, int* cell, int natom, int ncell)
{        
    int blockDim = 256;
    int gridDim = (natom + blockDim - 1) / blockDim;
    gridDim = (gridDim>1024)? 1024 : gridDim;
    hipLaunchKernelGGL(hipKernelCellCounts, gridDim, blockDim, 0, 0, cellcounts, cell, natom, ncell);
}

void hipCell2AtomList(int *c2alist, int *c2anumsum, int *c2anum, int *clist, int *d_temp, int natom, int ncell)
{           
    // sort atom-to-cell list    
    size_t  temp_storage_bytes  = 0;
    void  *d_temp_storage = NULL;
    hipcub::DeviceRadixSort::SortPairs(d_temp_storage, temp_storage_bytes, clist, c2anum, c2anumsum, c2alist, natom);
            
    //hipMalloc( (void**)&d_temp_storage, temp_storage_bytes);
    hipIndexInit(c2anumsum, natom);
    hipcub::DeviceRadixSort::SortPairs((void*) d_temp, temp_storage_bytes, clist, c2anum, c2anumsum, c2alist, natom);

    // count number of atoms for every cell
    hipCellCounts(c2anumsum, c2anum, natom, ncell);     
}

template <typename T> 
__global__ void hipKernelFullNeighborList2D(int *neighborlist, int *neighbornum, T *x, T *rcutsq, int *alist, 
...
template <typename T> void hipFullNeighborList2D(int *neighborlist, int *neighbornum, T *x, T *rcutsq,
...
{                        
    ...
    hipLaunchKernelGGL(hipKernelFullNeighborList2D, gridDim, blockDim, 0, 0, neighborlist, neighbornum, 
            x, rcutsq, alist, clist, c2alist, c2anumsum, nc, inum, jnum, dim);
}
template void hipFullNeighborList2D(int*, int*, double*, double*, int*, int*, int*, int*, int*, int, int, int);
template void hipFullNeighborList2D(int*, int*, float*, float*, int*, int*, int*, int*, int*, int, int, int);

template <typename T> 
__global__ void hipKernelFullNeighborList2D(int *neighborlist, int *neighbornum, T *x, T *rcutsq, 
...
template void hipFullNeighborList2D(int*, int*, double*, double*, int, int, int, int);
template void hipFullNeighborList2D(int*, int*, float*, float*, int, int, int, int);
