...
template <typename T> __global__ void gpuKernelSphericalHarmonicsBesselWithDeriv(T *Sr, T *Si, 
        T *Srx, T *Six, T *Sry, T *Siy, T *Srz, T *Siz, T *x, T *y, T *z, T *x0, 
        T *P, T *tmp, T *f, T *dP, T *dtmp, T *df, T *fac, T pi, int L, int K, int N)
{                        
    // L                      : the maximum degree of spherical harmonics
    // K                      : the maximum degree of spherical Bessel functions
    // N                      : length of x, y, z
...
template <typename T> void gpuSphericalHarmonicsBesselWithDeriv(T *Sr, T *Si, T *Srx, T *Six, T *Sry, T *Siy, T *Srz, T *Siz, 
      T *x, T *y, T *z, T *x0, T *P, T *tmp, T *f, T *dP, T *dtmp, T *df, T *fac, T pi, int L, int K, int N)
{        
    int blockDim = 256;
    int gridDim = (N + blockDim - 1) / blockDim;
...
template <typename T> void gpuSphericalHarmonicsBesselWithDeriv(T *Sr, T *Si, T *Srx, T *Six, T *Sry, T *Siy, T *Srz, T *Siz, 
      T *xij, T *x0, T *P, T *tmp, T *f, T *dP, T *dtmp, T *df, T *fac, T pi, int L, int K, int N)
{        
    int blockDim = 256;
    int gridDim = (N + blockDim - 1) / blockDim;
    gridDim = (gridDim>1024)? 1024 : gridDim;
...
// core function
template <typename T> __global__ void gpuKernelRadialSphericalHarmonicsPowerDeriv2(T *pd, T *ar, T *ai, T *arx, T *aix, 
       T *ary, T *aiy, T *arz, T *aiz, int *indk, int *Nnb, int L, int K, int N, int K2, int Na, int N2, int N3)
{   
    // L                      : the maximum degree of spherical harmonics
    // K                      : the maximum degree of spherical Bessel functions    
    ...
    while (tid < N3) { 
        ...
        for (int i=0; i<numnb; i++) {// loop over each neighbor of atom n                         
            ...
            for (int m=0; m<(2*l+1); m++) {
                ...
                pd[3*j+0] += ar[i1]*ary[j2] + ary[i2]*ar[j1] + ai[i1]*aiy[j2] + aiy[i2]*ai[j1];
                pd[3*j+0] += ar[i1]*arz[j2] + arz[i2]*ar[j1] + ai[i1]*aiz[j2] + aiz[i2]*ai[j1];                    
            }
        }
        tid += blockDim.x * gridDim.x;
    }                         
...
{        
    ...
    int gridDim = (N3 + blockDim - 1) / blockDim;
    gridDim = (gridDim>1024)? 1024 : gridDim;
    gpuKernelRadialSphericalHarmonicsPowerDeriv2<<<gridDim, blockDim>>>(pd, ar, ai, arx, aix, 
            ary, aiy, arz, aiz, indk, Nnb, L, K, N, K2, N1, N2, N3);
}
template void gpuRadialSphericalHarmonicsPowerDeriv2(double*, double*, double*, double*, 
        double*, double*, double*, double*, double*, int*, int*, int, int, int);
template void gpuRadialSphericalHarmonicsPowerDeriv2(float*, float*, float*, float*,  
        float*, float*, float*, float*, float*, int*, int*, int, int, int);
...
// core function
template <typename T> __global__ void gpuKernelRadialSphericalHarmonicsBispectrumDeriv2(T *bd, 
        T *ar, T *ai, T *arx, T *aix, T *ary, T *aiy, T *arz, T *aiz, T*cg, int *indk, int *indl,
        int *indm, int *rowm, int *Nnb, int Na, int Nub, int Ncg, int K, int N, int K2, int N2, int N3)
{   
    // K                      : the maximum degree of spherical Bessel functions
...
template <typename T> void gpuRadialSphericalHarmonicsBispectrumDeriv2(T *bd, 
        T *ar, T *ai, T *arx, T *aix, T *ary, T *aiy, T *arz, T *aiz, T*cg, int *indk, int *indl,
        int *indm, int *rowm, int *Nnb, int Na, int Nub, int Ncg, int K)
{        
    int N = Nnb[Na]-Nnb[0]; // total number of neighbors 
...
// core function
template <typename T> __global__ void gpuKernelRadialSphericalHarmonicsBasisDeriv2(T *dd, T *cd, 
        int *atomtype, int *neighlist, int *Nnb, int Ntype, int Na, int Nbf, int N, int N2, int N3)
{       
    // Ntype                : number of atom types 
    // Na                   : number of atoms in the simulation domain 
    // Nbf                  : number of basis functions per atom type
    ...
    // c [Na*Nbf]           : spectrum components for all atoms
    // d [Nbf*Ntype]        : basis functions based on atom types
    // cd [3*N*Nbf]         : derivatives of spectrum components  
    // dd [3*Na*Nbf*Ntype]  : derivatives of the basis functions
    ...
    while (tid < N3) { 
        ...
        for (int q=0; q<numnb; q++) { // loop over each neighbor of atom n                    
            if (tn==t) {// type of atom n is equal to t
                // atom n is self, atom i is neighbor
                int qnm = m*N + (Nnb[n] + q);  // index of the spectrum components of the atom pair (i, n)                    
                dd[3*nmt+0] +=  -cd[3*qnm+0];
                dd[3*nmt+1] +=  -cd[3*qnm+1];
                dd[3*nmt+2] +=  -cd[3*qnm+2]; 
            }                    
            ...
            if (ti==t) { // type of atom i is equal to t                    
                ...
                int rim = m*N + (Nnb[i] + r); // index of the spectrum components of the atom pair (n, i)                                        
                // atom n is neighbor, atom i is self
                dd[3*nmt+0] += cd[3*rim+0];
                dd[3*nmt+1] += cd[3*rim+1];
                dd[3*nmt+2] += cd[3*rim+2];
            }
        }                
        tid += blockDim.x * gridDim.x;                
    }
...
{            
    ...
    int gridDim = (N3 + blockDim - 1) / blockDim;
    gridDim = (gridDim>1024)? 1024 : gridDim;
    gpuKernelRadialSphericalHarmonicsBasisDeriv2<<<gridDim, blockDim>>>(dd, cd, atomtype, 
            neighlist, Nnb, Ntype, Na, Nbf, N, N2, N3);
}
template void gpuRadialSphericalHarmonicsBasisDeriv2(double*, double*, int*, int*, int*, int, int, int);
template void gpuRadialSphericalHarmonicsBasisDeriv2(float*, float*, int*, int*, int*, int, int, int);
