...
if strcmpi(app.configfile,"")==0 && strcmpi(app.dataformat,"")==0
    config = readconfigfile(app);
    app.nconfigs = config.nconfigs;
elseif strcmpi(app.configfile,"")==0 && app.configmode>=0
...
app.nceta = length(app.eta); % length of eta
...
app.natomtype = max(length(app.atommasses), length(app.atomtype));
...
app.flag = [app.descriptor app.spectrum app.training app.runMD app.potentialform app.neighpair...
            ...
            app.dftdata app.unitstylenum app.ensemblemode app.neighcheck];
