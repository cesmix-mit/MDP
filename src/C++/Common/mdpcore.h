        ...
        int *ai, int *aj, int *ti, int *tj, int twojmax, int idxu_max, int ijnum, int backend)
{
    if (backend == 1)
        cpuComputeUij(ulist_r, ulist_i, rootpqarray, rij, radelem, rmin0, rfac0, rcutfac, idxu_block, 
                        ai, aj, ti, tj, twojmax, idxu_max, ijnum);
#ifdef USE_OMP  
    if (backend == 4)
...
inline void SnapComputeUi(dstype *Stotr, dstype *Stoti, dstype *rootpqarray, dstype *rij, dstype *wjelem, dstype *radelem, 
        dstype rmin0, dstype rfac0, dstype rcutfac, int *idxu_block, int *map, int *ai, int *ti, int *tj, 
        int twojmax, int idxu_max, int inum, int ijnum, int switchflag, int chemflag, int backend)
{
    if (backend == 1)
        cpuSnapComputeUi(Stotr, Stoti, rootpqarray, rij, wjelem, radelem, rmin0, rfac0, rcutfac, 
                idxu_block, map, ai, ti, tj, twojmax, idxu_max, inum, ijnum, switchflag, chemflag);
#ifdef USE_OMP  
    if (backend == 4)
        ompSnapComputeUi(Stotr, Stoti, rootpqarray, rij, wjelem, radelem, rmin0, rfac0, rcutfac, 
                idxu_block, map, ai, ti, tj, twojmax, idxu_max, inum, ijnum, switchflag, chemflag);
#endif                                
#ifdef USE_HIP  
    if (backend == 3)
        hipSnapComputeUi(Stotr, Stoti, rootpqarray, rij, wjelem, radelem, rmin0, rfac0, rcutfac, 
                idxu_block, map, ai, ti, tj, twojmax, idxu_max, inum, ijnum, switchflag, chemflag);
#endif                                        
#ifdef USE_CUDA   
    if (backend == 2)
        gpuSnapComputeUi(Stotr, Stoti, rootpqarray, rij, wjelem, radelem, rmin0, rfac0, rcutfac, 
                idxu_block, map, ai, ti, tj, twojmax, idxu_max, inum, ijnum, switchflag, chemflag);
#endif                          
}


inline void AddWself2Ui(dstype *ulisttot_r, dstype *ulisttot_i, dstype wself, int *idxu_block, int *type,
        int *map, int *ai, int wselfall_flag, int chemflag, int idxu_max, int nelements, 
        int twojmax, int inum, int backend)
{
    if (backend == 1)
        cpuAddWself2Ui(ulisttot_r, ulisttot_i, wself, idxu_block, type,
                    map, ai, wselfall_flag, chemflag, idxu_max, nelements, twojmax, inum);
#ifdef USE_OMP  
    if (backend == 4)
        ompAddWself2Ui(ulisttot_r, ulisttot_i, wself, idxu_block, type,
                    map, ai, wselfall_flag, chemflag, idxu_max, nelements, twojmax, inum);
#endif                                
#ifdef USE_HIP  
    if (backend == 3)
        hipAddWself2Ui(ulisttot_r, ulisttot_i, wself, idxu_block, type,
                    map, ai, wselfall_flag, chemflag, idxu_max, nelements, twojmax, inum);
#endif                                        
#ifdef USE_CUDA   
    if (backend == 2)
        gpuAddWself2Ui(ulisttot_r, ulisttot_i, wself, idxu_block, type,
                    map, ai, wselfall_flag, chemflag, idxu_max, nelements, twojmax, inum);
#endif                          
}

inline void SnapComputeBi(dstype *blist, dstype *ulisttot_r, dstype *ulisttot_i, 
        dstype *cglist, dstype *bzero, int *idxb, int *idxcg_block, int *idxu_block, int *idxz_block, int twojmax, 
        int idxb_max, int idxu_max, int idxz_max, int nelements, int bnorm_flag, int bzero_flag, int wselfall_flag, 
        int inum, int backend)
{
    if (backend == 1)
        cpuSnapComputeBi(blist, ulisttot_r, ulisttot_i, cglist, bzero, idxb, idxcg_block, idxu_block, idxz_block, 
                twojmax, idxb_max, idxu_max, idxz_max, nelements,  bnorm_flag, bzero_flag, wselfall_flag, inum);
#ifdef USE_OMP  
    if (backend == 4)
        ompSnapComputeBi(blist, ulisttot_r, ulisttot_i, cglist, bzero, idxb, idxcg_block, idxu_block, idxz_block, 
                twojmax, idxb_max, idxu_max, idxz_max, nelements,  bnorm_flag, bzero_flag, wselfall_flag, inum);
#endif                                
#ifdef USE_HIP  
    if (backend == 3)
        hipSnapComputeBi(blist, ulisttot_r, ulisttot_i, cglist, bzero, idxb, idxcg_block, idxu_block, idxz_block, 
                twojmax, idxb_max, idxu_max, idxz_max, nelements,  bnorm_flag, bzero_flag, wselfall_flag, inum);
#endif                                        
#ifdef USE_CUDA   
    if (backend == 2)
        gpuSnapComputeBi(blist, ulisttot_r, ulisttot_i, cglist, bzero, idxb, idxcg_block, idxu_block, idxz_block, 
                twojmax, idxb_max, idxu_max, idxz_max, nelements,  bnorm_flag, bzero_flag, wselfall_flag, inum);
#endif                          
}

inline void SnapComputeEi(dstype *eatom, dstype *ulisttot_r, dstype *ulisttot_i, 
        dstype *cglist, dstype *bzero, dstype *coeffelem, int *ilist, int *map, int *type, int *idxb, int *idxcg_block, 
        int *idxu_block, int twojmax, int idxb_max, int idxu_max, int nelements, 
        int ncoeffall, int bnorm_flag, int bzero_flag, int wselfall_flag, int quadraticflag, int inum, int backend)
{
    if (backend == 1)
        cpuSnapComputeEi(eatom, ulisttot_r, ulisttot_i, cglist, bzero, coeffelem, ilist, map, type, 
                idxb, idxcg_block, idxu_block,  twojmax, idxb_max, idxu_max, nelements,  
                ncoeffall, bnorm_flag, bzero_flag, wselfall_flag, quadraticflag, inum);
#ifdef USE_OMP  
    if (backend == 4)
        ompSnapComputeEi(eatom, ulisttot_r, ulisttot_i, cglist, bzero, coeffelem, ilist, map, type, 
                idxb, idxcg_block, idxu_block,  twojmax, idxb_max, idxu_max, nelements,  
                ncoeffall, bnorm_flag, bzero_flag, wselfall_flag, quadraticflag, inum);
#endif                                
#ifdef USE_HIP  
    if (backend == 3)
        hipSnapComputeEi(eatom, ulisttot_r, ulisttot_i, cglist, bzero, coeffelem, ilist, map, type, 
                idxb, idxcg_block, idxu_block,  twojmax, idxb_max, idxu_max, nelements,  
                ncoeffall, bnorm_flag, bzero_flag, wselfall_flag, quadraticflag, inum);
#endif                                        
#ifdef USE_CUDA   
    if (backend == 2)
        gpuSnapComputeEi(eatom, ulisttot_r, ulisttot_i, cglist, bzero, coeffelem, ilist, map, type, 
                idxb, idxcg_block, idxu_block,  twojmax, idxb_max, idxu_max, nelements,  
                ncoeffall, bnorm_flag, bzero_flag, wselfall_flag, quadraticflag, inum);
#endif                          
}

inline void SnapComputeYi(dstype *ylist_r, dstype *ylist_i, dstype *ulisttot_r, dstype *ulisttot_i, 
...
{
    if (backend == 1)
        cpuSnapComputeYi(ylist_r, ylist_i, ulisttot_r, ulisttot_i, cglist, betalist, idxz, idxb_block, idxu_block, 
                    idxcg_block, twojmax, idxb_max, idxu_max, idxz_max, nelements, bnorm_flag, inum);
#ifdef USE_OMP  
    if (backend == 4)
        ompSnapComputeYi(ylist_r, ylist_i, ulisttot_r, ulisttot_i, cglist, betalist, idxz, idxb_block, idxu_block, 
                    idxcg_block, twojmax, idxb_max, idxu_max, idxz_max, nelements, bnorm_flag, inum);
#endif                                
#ifdef USE_HIP  
    if (backend == 3)
        hipSnapComputeYi(ylist_r, ylist_i, ulisttot_r, ulisttot_i, cglist, betalist, idxz, idxb_block, idxu_block, 
                    idxcg_block, twojmax, idxb_max, idxu_max, idxz_max, nelements, bnorm_flag, inum);
#endif                                        
#ifdef USE_CUDA   
    if (backend == 2)
        gpuSnapComputeYi(ylist_r, ylist_i, ulisttot_r, ulisttot_i, cglist, betalist, idxz, idxb_block, idxu_block, 
                    idxcg_block, twojmax, idxb_max, idxu_max, idxz_max, nelements, bnorm_flag, inum);
#endif                          
}

inline void SnapComputeYi(dstype *ylist_r, dstype *ylist_i, dstype *ulisttot_r, dstype *ulisttot_i, 
        dstype *cglist, dstype *betalist, int *map, int *type, int *idxz, int *idxb_block, int *idxu_block, 
        int *idxcg_block,  int twojmax, int idxb_max, int idxu_max, int idxz_max, int nelements, 
        int ncoeffall, int bnorm_flag, int inum, int backend)
{
    if (backend == 1)
        cpuSnapComputeYi(ylist_r, ylist_i, ulisttot_r, ulisttot_i, cglist, betalist, map, type, idxz, idxb_block, 
               idxu_block, idxcg_block, twojmax, idxb_max, idxu_max, idxz_max, nelements, ncoeffall, bnorm_flag, inum);
#ifdef USE_OMP  
    if (backend == 4)
        ompSnapComputeYi(ylist_r, ylist_i, ulisttot_r, ulisttot_i, cglist, betalist, map, type, idxz, idxb_block, 
               idxu_block, idxcg_block, twojmax, idxb_max, idxu_max, idxz_max, nelements, ncoeffall, bnorm_flag, inum);
#endif                                
#ifdef USE_HIP  
    if (backend == 3)
        hipSnapComputeYi(ylist_r, ylist_i, ulisttot_r, ulisttot_i, cglist, betalist, map, type, idxz, idxb_block, 
               idxu_block, idxcg_block, twojmax, idxb_max, idxu_max, idxz_max, nelements, ncoeffall, bnorm_flag, inum);
#endif                                        
#ifdef USE_CUDA   
    if (backend == 2)
        gpuSnapComputeYi(ylist_r, ylist_i, ulisttot_r, ulisttot_i, cglist, betalist, map, type, idxz, idxb_block, 
               idxu_block, idxcg_block, twojmax, idxb_max, idxu_max, idxz_max, nelements, ncoeffall, bnorm_flag, inum);
#endif                          
}

inline void SnapComputeFi(dstype *fatom, dstype *vatom, dstype *ylist_r, dstype *ylist_i, dstype *rootpqarray, dstype *rij, 
        dstype *wjelem, dstype *radelem,  dstype rmin0, dstype rfac0, dstype rcutfac, int *idxu_block, 
        int *map, int *aii, int *ai, int *aj, int *ti, int *tj, int twojmax, int idxu_max, int inum, int anum, int ijnum, 
        int switchflag, int chemflag, int backend)        
{
    if (backend == 1)
        cpuSnapComputeFi(fatom, vatom, ylist_r, ylist_i, rootpqarray, rij, wjelem, radelem, rmin0, rfac0, rcutfac, 
                idxu_block, map, aii, ai, aj, ti, tj, twojmax, idxu_max, inum, anum, ijnum, switchflag, chemflag); 
#ifdef USE_OMP  
    if (backend == 4)
        ompSnapComputeFi(fatom, vatom, ylist_r, ylist_i, rootpqarray, rij, wjelem, radelem, rmin0, rfac0, rcutfac, 
                idxu_block, map, aii, ai, aj, ti, tj, twojmax, idxu_max, inum, anum, ijnum, switchflag, chemflag); 
#endif                                
#ifdef USE_HIP  
    if (backend == 3)
        hipSnapComputeFi(fatom, vatom, ylist_r, ylist_i, rootpqarray, rij, wjelem, radelem, rmin0, rfac0, rcutfac, 
                idxu_block, map, aii, ai, aj, ti, tj, twojmax, idxu_max, inum, anum, ijnum, switchflag, chemflag); 
#endif                                        
#ifdef USE_CUDA   
    if (backend == 2)
        gpuSnapComputeFi(fatom, vatom, ylist_r, ylist_i, rootpqarray, rij, wjelem, radelem, rmin0, rfac0, rcutfac, 
                idxu_block, map, aii, ai, aj, ti, tj, twojmax, idxu_max, inum, anum, ijnum, switchflag, chemflag); 
#endif                          
}
...
inline void ComputeDbidrj(dstype *dblist, dstype *zlist_r, dstype *zlist_i, dstype *dulist_r,
        dstype *dulist_i, int *idxb, int *idxu_block, int *idxz_block, int *map, 
        int *ai, int *aj, int *ti, int *tj, int twojmax, int idxb_max, int idxu_max, int idxz_max, int nelements,
        int bnorm_flag, int chemflag, int ijnum, int backend)
{
    if (backend == 1)
        cpuComputeDbidrj(dblist, zlist_r, zlist_i, dulist_r, dulist_i, idxb, idxu_block, 
                idxz_block, map, ai, aj, ti, tj, twojmax, idxb_max, idxu_max, idxz_max, 
...
{
    if (backend == 1)
        cpuSnapTallyForceFull(fatom, fij, ai, aj, alist, ijnum);
#ifdef USE_OMP  
    if (backend == 4)
        ompSnapTallyForceFull(fatom, fij, ai, aj, alist, ijnum);
#endif                                
#ifdef USE_HIP  
    if (backend == 3)
        hipSnapTallyForceFull(fatom, fij, ai, aj, alist, ijnum);
#endif                                        
#ifdef USE_CUDA   
    if (backend == 2)
...
{
    if (backend == 1)
        cpuSnapTallyVirialFull(vatom, fij, rij, ai, aj, inum, ijnum);
#ifdef USE_OMP  
    if (backend == 4)
        ompSnapTallyVirialFull(vatom, fij, rij, ai, aj, inum, ijnum);
#endif                                
#ifdef USE_HIP  
    if (backend == 3)
        hipSnapTallyVirialFull(vatom, fij, rij, ai, aj, inum, ijnum);
#endif                                        
#ifdef USE_CUDA   
    if (backend == 2)
...
inline void NeighborPairs(dstype *xij, dstype *x, int *aii, int *ai, int *aj,  
      ...
      int *atomtype, int *alist, int inum, int jnum, int dim, int backend)
{
    if (backend == 1)
