...
void CRegression::PotentialDescriptors(CCalculation &CCal)
{    
    int dim = CCal.common.dim;    
    int backend = CCal.common.backend;    
    int N = CCal.common.Nempot;  
    int M = CCal.common.Ncoeff;    
    int inummax = CCal.common.inummax;
    int *nparam = CCal.common.nmu;        
    int nc = CCal.config.ndims[0];

    dstype *ai, *ad, *av, *q, *bi, *bd, *bv;    
    dstype *x = CCal.sys.x;    
    dstype *param = CCal.app.muep;
    
    if (N>0) {
        TemplateMalloc(&ai, N, backend);  
        TemplateMalloc(&ad, inummax*dim*N, backend);  
        TemplateMalloc(&av, 6*N, backend);      
    }    
    if (M>0) {
        TemplateMalloc(&bi, M, backend);  
        TemplateMalloc(&bd, inummax*dim*M, backend);  
        TemplateMalloc(&bv, 6*M, backend);  
    }
    
    // open files
    string file1 = CCal.common.fileout + "_p" + NumberToString(CCal.common.mpiRank) + "_ai.bin";
    string file2 = CCal.common.fileout + "_p" + NumberToString(CCal.common.mpiRank) + "_ad.bin";
    string file3 = CCal.common.fileout + "_p" + NumberToString(CCal.common.mpiRank) + "_av.bin";
    ...
    for (int ci=0; ci<nc; ci++) { // loop over each configuration             
        ...
        CCal.PotentialDescriptors(ai, ad, av, bi, bd, bv, x, param, nparam);                
        
        // save data into files
        if (N > 0) {
            writearray(out1, ai, N, backend);    
            writearray(out2, ad, inum*dim*N, backend);    
            writearray(out3, av, 6*N, backend);    
        }
        if (M > 0) {
        ...
        //printf("Configuration %i, # atoms %i, # empirical descriptors %i, # ML descriptors %i\n", ci+1, inum, N, M);
    }    
    ...
    if (N>0) {
        ...
        TemplateFree(ad, backend);  
        TemplateFree(av, backend);      
    }
    if (M > 0) {
