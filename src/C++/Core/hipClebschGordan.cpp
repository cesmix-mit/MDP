...
#ifndef __HIPCLEBSCHGORDAN
#define __HIPCLEBSCHGORDAN 

void hipGetIndk(int *indk, int K)
{               
    ...
    // chipute indices for symmetric tensor products of spherical Bessel functions
...
{               
    // chipute the Clebsch-Gordan coefficient for two tuple pairs (j2, j1, j) and (m2, m1, m)
    
    T C;
    
    int jm = (j1-j2) > (j2-j1) ? (j1-j2) : (j2-j1); //max(j1-j2,j2-j1);
    ...
    if ((m1+m2-m != 0) || (j < jm)  || (j > j1+j2)) { // zero Clebsch-Gordan coefficient 
        ...
        int k1 = 0 > (j2-j-m1) ? 0 : (j2-j-m1);                
        k1 = k1 > (j1-j+m2) ? k1 : (j1-j+m2);
        int k2 = (j1+j2-j) < (j1-m1) ? (j1+j2-j) : (j1-m1);
        k2 = k2 < (j2+m2) ? k2 : (j2+m2);
        int n = k2-k1+1;        
        C = 0.0;
        for (int i=0; i<n; i++) {
            int k = k1 + i;        
...
{                   
    ...
    // chipute the total number of non-zero Clebsch-Gordan coefficients for N tuples (l2, l1, l)
    ...
    for (int n=0; n<N; n++) { // loop over each tuple (l2, l1, l)
        ...
        int lm = (l1-l2) > (l2-l1) ? (l1-l2) : (l2-l1);        
...
{                   
    // M             : the total number of non-zero Clebsch-Gordan coefficients chiputed in the previous functions
    // N             : the number of tuples (l2, l1, l)
...
// core function
template <typename T> void hipSphericalHarmonicsAtomSum(T *Ylmr, T *Ylmi, T *x, T *y, T *z, 
...
{                            
    ...
    // Chipute spherical harmonics functions, sum_i Y_{lm}(x_i,y_i,z_i), for l = 0,1,...,L and m = -l,...,0,...,l        
    //  l = 0:                0
    //  l = 1:              1 2 3
    //  l = 2:            4 5 6 7 8
    ...
    for (int i=0; i<N; i++) {  // loop over each neighbor atom                             
        ...
        // Spherical harmonics for l > 1
        for (l=2; l<=L; l++) {                                        
            // Chipute associated Legendre polynomial using recurrence formula
            ...
            // chipute spherical harmonics at level l for m = 0, 1, ..., l
            for (m=0; m<=l; m++) {
                j = (l*l + l + m);
                C = sqrt((2*l+1)*fac[l-m]/(4*pi*fac[l+m])); 
    ...
    // Chipute the spherical harmonics for m < 0 using symmetry properties
    for (int l=1; l<=L; l++) {
        T C = -1.0;
        for (int m=1; m<=l; m++)  {                 
...
template void hipSphericalHarmonicsAtomSum(double*, double*, double*, double*, double*, double*, double*, double*, double, int, int);
template void hipSphericalHarmonicsAtomSum(float*, float*, float*, float*, float*, float*, float*, float*, float, int, int);

// core function
template <typename T> void hipSphericalHarmonicsBispectrum(T *b, T *Ylmr, T *Ylmi, T *fac, int L)
{                   
    // L                    : the maximum degree of spherical harmonics
    ...
    // chipute the bispectrum coeffcients for spherical harmonic functions
...
template void hipSphericalHarmonicsBispectrum(double*, double*, double*, double*, int);
template void hipSphericalHarmonicsBispectrum(float*, float*, float*, float*, int);

// core function
template <typename T> int hipSphericalHarmonicsBispectrumIndex(T *b, int L)
{               
    // L : the maximum degree of spherical harmonics    
    // b : the bispectrum coeffcients of spherical harmonic functions chiputed by the above function
    // chipute the number of non-zero unqiue bispectrum coeffcients for spherical harmonic functions
    
    // Symmetry conditions for the bispectrum chiponents
    // [l2 l1 l]
    // [l1 l2 l]
    // [l l2 l1]
    // [l2 l l1]
    // [l1 l l2]
    // [l l1 l2]
    // Use the symmetry conditions to remove zero and duplicate bispectrum chipoments
...
template int hipSphericalHarmonicsBispectrumIndex(double *, int);
template int hipSphericalHarmonicsBispectrumIndex(float *, int);

// core function
template <typename T> void hipSphericalHarmonicsBispectrumIndex(int *indl, T *b, int M, int L)
{    
    // L    : the maximum degree of spherical harmonics    
    ...
    // chipute the indices of non-zero unqiue bispectrum coeffcients for spherical harmonic functions
...
template void hipSphericalHarmonicsBispectrumIndex(int*, double *, int, int);
template void hipSphericalHarmonicsBispectrumIndex(int*, float *, int, int);
